...
The binary generated by building `build.jai` takes an entry file to parse as the first and only argument it generates an `out` folder with JSON files representing all parsed files. The JSON export has been used for quick visualization of the output while developing the parser. The parser gonna be used as a Jai Module (without exporting to JSON).
